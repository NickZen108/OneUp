...
function setMissionStatus(id, status) { if (!missions.items[id]) return; console.log?.('mission valgt', { missionId: id, status }); if (status === 'active' && missions.items[id].progress >= residentTemplates[id].goal) { missions.items[id].progress = 0; missions.items[id].allocations = []; missions.items[id].bonusAwarded = false; missions.items[id].completedAt = null; } if (missions.activeId && missions.activeId !== id && status === 'active') missions.items[missions.activeId].status = 'paused'; missions.activeId = status === 'active' ? id : (missions.activeId === id ? null : missions.activeId); missions.items[id].status = status; saveMissions(); updateAllViews(); }
...
async function startMission(id, mode = 'pause', options = {}) {
  ...
  if (options.goToActivity) { showPage('activity'); renderCompactMissionVillage(); scrollToCompactMissionVillage(); }
  if (options.scrollToMission !== false && !options.goToActivity) {
...
function compactMissionStatusText(id) {
  const messages = {
    buster: 'Buster venter spændt på sin tur.',
    olsen: 'Fru Olsen er klar til at komme ud og få mere energi.',
    hansen: 'Hr. Hansen prøver at finde mere ro.',
  };
  const m = missions.items[id];
  const t = residentTemplates[id];
  if (m.status === 'completed' || m.progress >= t.goal) return `${t.name} har fået hjælp. ${t.thanks}`;
  if (m.progress > 0) return getMissionMessage(id);
  return messages[id] || getMissionMessage(id);
}
function compactMissionVillageHtml() {
  const active = getActiveMission();
  if (!active) return `<div class="compact-mission-empty"><h2>Aktiv missionslandsby</h2><p>Vælg en mission og hjælp en beboer gennem din aktivitet.</p><button type="button" class="complete-button" data-nav="missions">Vælg mission</button></div>`;
  const id = active.id, t = residentTemplates[id];
  const value = Math.min(active.progress, t.goal);
  const unit = t.type === 'steps' ? 'skridt' : 'åndedrætsrunder';
  return `<div class="compact-mission-scene compact-mission-${id}">
    <div class="compact-resident" aria-hidden="true"><span class="compact-resident-face">${t.emoji}</span><span class="compact-resident-name">${t.name}</span></div>
    <div class="compact-mission-copy"><p class="eyebrow">Aktiv missionslandsby</p><h2>${t.mission}</h2><p><strong>Humør:</strong> ${getResidentMood(id)}</p><p class="compact-progress-text"><strong>${missionProgressText(id)}</strong></p><progress max="${t.goal}" value="${value}" aria-label="${value.toLocaleString('da-DK')} af ${t.goal.toLocaleString('da-DK')} ${unit}"></progress><p>${compactMissionStatusText(id)}</p><p class="mission-missing">${missionMissingText(id)}</p></div>
    <div class="button-row compact-mission-actions"><button type="button" data-compact-action="resident" data-mission-id="${id}">Se beboer</button><button type="button" data-mission-action="pause" data-mission-id="${id}">Pause mission</button><button type="button" data-nav="missions">Skift mission</button></div>
  </div>`;
}
function renderCompactMissionVillage() { if (elements.activeMissionActivity) elements.activeMissionActivity.innerHTML = compactMissionVillageHtml(); }
...
function updateMissionsView() { renderCompactMissionVillage(); if (elements.missionList) { const ids = Object.keys(missions.items).sort((a, b) => (missions.activeId === b) - (missions.activeId === a)); elements.missionList.innerHTML = ids.map(missionCardHtml).join('') + '<p class="info-box">Flere aktiviteter som søvn, løb, cykling og naturture kommer senere.</p>'; } }
...
function isElementReadyForScroll(element) { return !!(element && !element.hidden && (element.isConnected !== false)); }
function findMissionDetailsElement(id) { return byId(`mission-details-${id}`) || $(`[data-mission-card=\"${id}\"]`); }
function scrollToMissionDetails(id) { const card = findMissionDetailsElement(id); console.log?.('detaljeelement fundet', { missionId: id, found: !!card }); if (!isElementReadyForScroll(card)) return false; card.scrollIntoView?.({ behavior: 'smooth', block: 'start' }); console.log?.('scroll udført', { missionId: id }); card.focus?.({ preventScroll: true }); highlightMissionDetails(card); return true; }
function scrollToCompactMissionVillage(attempt = 0) { const run = () => { const box = elements.compactMissionVillage || byId('compact-mission-village'); const section = box?.closest?.('.app-section') || $('[data-page="activity"]'); if (section?.dataset?.page && section.hidden) showPage(section.dataset.page); if (isElementReadyForScroll(box)) { box.scrollIntoView?.({ behavior: 'smooth', block: 'start' }); box.focus?.({ preventScroll: true }); } else if (attempt < 3) window.setTimeout?.(() => scrollToCompactMissionVillage(attempt + 1), 60); }; if (typeof window.requestAnimationFrame === 'function') window.requestAnimationFrame(() => window.setTimeout?.(run, 0)); else window.setTimeout?.(run, 60); }
function scheduleMissionDetailsScroll(id, attempt = 0) { const run = () => { const card = findMissionDetailsElement(id); const section = card?.closest?.('.app-section') || $('[data-page="missions"]'); if (section?.dataset?.page && section.hidden) showPage(section.dataset.page); if (!scrollToMissionDetails(id) && attempt < 3) window.setTimeout?.(() => scheduleMissionDetailsScroll(id, attempt + 1), 60); }; if (typeof window.requestAnimationFrame === 'function') window.requestAnimationFrame(() => window.setTimeout?.(run, 0)); else window.setTimeout?.(run, 60); }
...
function showPage(page) { console.log?.('fanen skiftet', { page }); $$('.app-section').forEach((s) => { s.hidden = s.dataset.page !== page; }); const world = elements.world || byId('world'); if (page === 'village' && world) { world.classList?.add('world', 'village'); if (world.style?.display === 'none') world.style.display = ''; } $$('.bottom-nav button').forEach((b) => b.classList.toggle('active', b.dataset.nav === page)); }
...
document.addEventListener?.('click', (event) => { try { const nav = event.target.closest?.('[data-nav]'); if (nav) showPage(nav.dataset.nav); const compact = event.target.closest?.('[data-compact-action]'); if (compact?.dataset.compactAction === 'resident') { showPage('village'); openResidentPanel(compact.dataset.missionId); } const btn = event.target.closest?.('[data-mission-action]'); if (btn) { const a = btn.dataset.missionAction, id = btn.dataset.missionId; if (a === 'start') void startMission(id, 'pause', { goToActivity: true }); else if (a === 'switch') void startMission(id, 'switch', { goToActivity: true }); else if (a === 'pause') { pauseMission(id); if (event.target.closest?.('#compact-mission-village')) scrollToCompactMissionVillage(); else scheduleMissionDetailsScroll(id); } else if (a === 'cancel') { cancelMission(id); scheduleMissionDetailsScroll(id); } else setMissionStatus(id, a); } } catch (error) { console.error('OneUp klik-handler fejlede', error); } });
